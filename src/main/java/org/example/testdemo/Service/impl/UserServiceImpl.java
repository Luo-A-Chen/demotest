...
public class UserServiceImpl implements UserService {
     ...
     * 构造器注入 - 推荐的方式
     */
    public UserServiceImpl(UserMapper userMapper, JwtUtil jwtUtil, FileStorage fileStorage) {
    ...
    public BaseResponse<String> uploadAvatar(MultipartFile avatarFile) {
        ...
        try (InputStream in = avatarFile.getInputStream()){
            ...
            String objectName = java.util.UUID.randomUUID().toString() + ext;
            String avatarUrl = fileStorage.store(objectName, in);
            // 如果用户已有头像，删除旧头像文件
            if (currentUser.getAvatar() != null && !currentUser.getAvatar().isEmpty()) {
                fileStorage.delete(currentUser.getAvatar());
            }
